    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST)) {
        ...
        if ($valid) {
			$query=$db->prepare ("UPDATE auto_user 
									SET user_nom = :name, user_prenom = :firstname,
		...
		$query = $db->prepare ("SELECT * FROM auto_user WHERE user_id = ".$id."");
        $query->execute();
...
?>
    	<div class="container">
			...
			<div class="row">
				<div class="col-sm-8">
					<h2>Vos Informations<br />
						<small>Ici vous pouvez mettre à jour vos données personnelles.</small>
					</h2>
				</div>
			</div>
			<hr>
